...
function housesImg(photo){
	...
	var item1='<img id="bigImg" src="'+url+'/upload/'+housesPhoto[0]+'" height="340" width="454" alt="">';
	$("#bigHousesPhoto").append(item1);
	var item="";
	for(var i=0;i<housesPhoto.length;i++){
		item +='<li><img src="'+url+'/upload/'+housesPhoto[i]+'" onclick="changeHousePhoto(this);" height="78" width="106" alt=""></li>';
	}
		...
		success: function(data){
			if(data.success){
			 $.each(data.data,function(i,houses) {
				 ...
				 var item='<li><div class="u-like-pic"><a href="'+url+'/article.html?id='+houses['id']+'"><img src="'+url+'/upload/'+photoStr+'" height="193" width="264" alt=""></a>'+
...
function changeHousePhoto(s){
	console.log(s);
	$("#bigImg").attr('src',s.src);
}
